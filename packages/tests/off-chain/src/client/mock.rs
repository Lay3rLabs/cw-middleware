...
impl MockTestClient {
    pub fn new(client: ContractTestClient) -> Self {
        ...
        let contract = ContractWrapper::new(
            cw_wavs_mock_service_manager::entry::execute,
            cw_wavs_mock_service_manager::entry::instantiate,
            cw_wavs_mock_service_manager::entry::query,
        );
        ...
        let contract = ContractWrapper::new(
            cw_wavs_mock_service_handler::entry::execute,
            cw_wavs_mock_service_handler::entry::instantiate,
            cw_wavs_mock_service_handler::entry::query,
        );
        let code_id = app.borrow_mut().store_code(Box::new(contract));
