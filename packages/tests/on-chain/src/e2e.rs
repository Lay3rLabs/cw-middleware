...
use crate::client::{TestClient, TestContractClient};
...
pub async fn run_e2e_tests(client: TestClient) {
    ...
    client.service_manager_exec().set_service_uri(service_url).await.unwrap();

    client.node.deploy_service(service.manager.clone()).await.unwrap();
    client.node.register_aggregator_service(&service).await.unwrap();
