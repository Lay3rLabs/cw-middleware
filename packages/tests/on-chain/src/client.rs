...
use crate::client::{config::TestConfig, contract::{BlsContractClient, EcdsaContractClient, MockContractClient}, node::WavsNodeClient};
use layer_climb::prelude::Address;
use mock_api::trigger::PushMessageEvent;
use utils::{contract_client::on_chain::WavsSigningPoolClient, prelude::*};
use wavs_types::{Component, ComponentSource, CosmosContractSubmission, Service, ServiceManager, Submit, Trigger, Workflow};
...
impl TestClient {
    ...
    pub async fn new_service(&self) -> Service {
        ...
        workflows.insert("messenger".parse().unwrap(), Workflow {
    ...
    pub fn service_handler_address(&self) -> Address {
        ...
        self.config.chain_config.parse_address(addr.as_str()).unwrap()
    }
